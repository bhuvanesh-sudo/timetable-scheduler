...
class TestModule4BackendAPI:
    ...
    def test_generate_schedule_unauthorized_user(self, api_client, faculty_user):
        """
        Test: Unauthorized (Role-based)
    ...
    def test_generate_schedule_success(self, api_client, admin_user, sample_data):
        """
        Test: POST generate
        Expected: Success (201 Created or 200 OK)
        Ensure admin users can trigger schedule generation with valid payload.
        """
        api_client.force_authenticate(user=admin_user)
        url = reverse('generate-schedule')
        payload = {
            'name': 'New Schedule Module 4',
            'semester': 'odd',
            'year': 1
        }
        response = api_client.post(url, payload)
        ...
        assert Schedule.objects.filter(name='New Schedule Module 4').exists()
    ...
    def test_generate_schedule_invalid_payload(self, api_client, admin_user):
        ...
        Ensure invalid payload (missing fields) returns 400.
        """
        api_client.force_authenticate(user=admin_user)
        url = reverse('generate-schedule')
